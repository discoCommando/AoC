...
main =
    H.makeMain [ pI |> toString, res1 |> toString, 'a' |> Char.toCode |> toString, 'z' |> Char.toCode |> toString, rotate 'a' 26 |> toString, res2 |> toString ]
...
pR room =
    room
        ...
        |> (\l ->
                let
                    checksum =
                        l |> H.at 1 |> String.toList |> List.reverse |> List.drop 1 |> List.reverse |> String.fromList
                ...
                in
                R (sId |> String.toInt |> Result.toMaybe |> H.uM) ns checksum
           )


pI =
    input |> String.lines |> List.map pR
...
checkRoom room =
    room.ns
        |> List.foldl (\x d -> x |> String.toList |> List.foldl pp d) Dict.empty
        |> Dict.toList
        |> Debug.log "asd"
        |> List.sortBy H.tf
        |> List.reverse
        ...
        |> String.fromList
        |> (\x ->
                case (x |> String.toList |> List.take 5 |> String.fromList) == room.checksum of
                    True ->
                        True
...
-- (x |> String.toList |> List.take 5 |> String.fromList, room.checksum)


res1 =
    pI |> List.map (\x -> ( x, checkRoom x )) |> List.filter H.ts |> List.map (H.tf >> .sId) |> List.sum


res2 =
    pI
        |> List.map (\x -> ( x, checkRoom x ))
        |> List.filter H.ts
        |> List.map
            (\m ->
                ( H.tf m
                , H.tf m |> (\x -> x.ns |> List.map (\a -> a |> String.toList |> List.map (\c -> rotate c x.sId) |> String.fromList) |> String.join " ")
                )
            )


dd =
    ('z' |> Char.toCode) - ('a' |> Char.toCode) + 1 |> Debug.log "dd"
...
rotate ch i =
    case ch of
        ...
        _ ->
            let
                ass =
                    (ch |> Char.toCode) - Char.toCode 'a'
            in
            ((ass + i) % dd) |> (+) (Char.toCode 'a') |> Char.fromCode
