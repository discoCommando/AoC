...
type Command
    = SetValue Int Int
    | Give Int OutputTaker OutputTaker
...
outputTaker : Parser OutputTaker
outputTaker =
    Parser.oneOf
        [ Parser.succeed Bot
            |. keyword "bot "
...
updateBot : Int -> Int -> State -> State
updateBot index value state =
    let
        newBots =
            state.bots
                |> Dict.update index
                    (\botState ->
                        case botState of
                            Just (Partial v) ->
                                H.iFF (v < value) (Full v value) (Full value v) |> Just
...
updateOutputTaker : OutputTaker -> Int -> State -> State
updateOutputTaker outputTaker value state =
    case outputTaker of
        Bot i ->
            updateBot i value state

        Output i ->
            { state | outputs = state.outputs |> Dict.insert i value }


step : Command -> State -> StepResult
step command state =
    case command of
        SetValue valueIndex botNumber ->
            let
                newState =
                    updateBot botNumber valueIndex state
            in
            case newState.bots |> Dict.get botNumber of
                Just (Full _ _) ->
                    newState |> GoBack
        ...
        Give botIndex output1 output2 ->
            case state.bots |> Dict.get botIndex of
                Nothing ->
                    Skipped
                ...
                Just botState ->
                    case botState of
                        ...
                        Full i1 i2 ->
                            state
                                |> updateOutputTaker output1 i1
                                |> updateOutputTaker output2 i2
                                |> GoBack


recurse : List Command -> List Command -> State -> State
recurse commandsLeft skippedCommandsReversed state =
    case commandsLeft of
        [] ->
            let
                _ =
                    Debug.log "skipped" skippedCommandsReversed
            in
            state

        command :: rest ->
            case step command state of
                Resolved newState ->
                    recurse rest skippedCommandsReversed newState

                Skipped ->
                    recurse rest (command :: skippedCommandsReversed) state

                GoBack newState ->
                    recurse (List.reverse skippedCommandsReversed ++ rest) [] newState
...
part1Answer =
    input
        ...
        |> Debug.log "parsed"
        |> (\commands -> recurse commands [] (State Dict.empty Dict.empty))
        |> Debug.log "result state"
        |> .bots
        |> Dict.toList
        |> List.filter (\( botNumber, botState ) -> botState == valueToFind)
        |> H.at 0
        |> H.tf
        |> Debug.log "part1 answer"
...
part2Answer =
    input
        ...
        |> .outputs
        |> (\outputs ->
                [ 0, 1, 2 ] |> List.map (\i -> Dict.get i outputs |> H.uM) |> List.foldl (*) 1
           )
        |> Debug.log "part2 answer"
