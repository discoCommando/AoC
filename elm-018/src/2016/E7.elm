...
import List
import List.Extra
import Parser exposing ((|.), (|=), keep, succeed, symbol)
import Regex
import String
...
pA =
    Parser.oneOf
        [ succeed I
            |. symbol "["
            |= keep Parser.oneOrMore Char.isLower
            |. symbol "]"
        , succeed O
            |= keep Parser.oneOrMore Char.isLower
        ]
...
pAs =
    Parser.repeat Parser.oneOrMore pA



-- pI a = a |> Regex.find Regex.All (Regex.regex "([a-z]+)\\[([a-z]+)\\]([a-z]+)")  |>
--         H.at 0 |> (.submatches >> List.map (H.uM)) |> (\r ->
--             X (r |> H.at 0) (r |> H.at 1) (r |> H.at 2)
--         )
...
pI a =
    a
        ...
        |> (\( a, b ) -> X (a |> List.map atox) (b |> List.map atox))


hasPal x =
    (x == List.reverse x) && (x /= (H.at 0 x |> List.repeat 4))


hasPalx a =
    if List.length a < 4 then
        False
    else
        case List.take 4 a |> hasPal of
            True ->
                True

            False ->
                hasPalx (List.drop 1 a)
...
getAba a =
    if List.length a < 3 then
        ...
        case a |> List.take 3 |> isAba of
            True ->
                (a |> List.take 3 |> String.fromList) :: getAba (List.drop 1 a)

            False ->
                getAba (List.drop 1 a)


isOk x =
    case x.is |> List.map (String.toList >> hasPalx) |> List.filter identity of
        _ :: _ ->
            False

        [] ->
            x.os |> List.map (String.toList >> hasPalx) |> List.filter identity |> List.isEmpty |> not


rev l =
    [ 1, 0, 1 ] |> List.map (H.at >> (|>) l) |> String.fromList


adasdas =
    rev ("xyx" |> String.toList) |> Debug.log "asd"


isOk2 x =
    x.os
        |> List.map (String.toList >> getAba)
        |> List.concat
        |> List.map (String.toList >> rev)
        |> List.map
            (\bab ->
                x.is |> List.map (\iss -> iss |> String.contains bab) |> List.member True
            )
        |> List.member True


res1 =
    input |> String.lines |> List.map pI |> Debug.log "ps" |> List.map isOk |> Debug.log "a" |> List.filter identity |> List.length
...
res2 =
    input |> String.lines |> List.map pI |> List.map isOk2 |> List.filter identity |> List.length
