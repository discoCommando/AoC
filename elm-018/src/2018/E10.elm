...
viewEntries model =
    let
        minX =
            model |> List.map .x |> List.foldl min 1000000000

        minY =
            model |> List.map .y |> List.foldl min 100000000

        maxX =
            model |> List.map .x |> List.foldl max -100000000

        maxY =
            model |> List.map .y |> List.foldl max -100000000
    in
    case maxX - minX < 300 && maxY - minY < 300 of
        True ->
            model
                |> print
...
update msg ( i, entries ) =
    ( i + 1, entries |> List.map move )
...
move : Entry -> Entry
move entry =
    { entry | x = entry.x + entry.vx, y = entry.y + entry.vy }


print : List Entry -> List String
print entries =
    let
        minX =
            entries |> List.map .x |> List.foldl min 1000000000

        minY =
            entries |> List.map .y |> List.foldl min 100000000

        maxX =
            entries |> List.map .x |> List.foldl max -100000000

        maxY =
            entries |> List.map .y |> List.foldl max -100000000

        mappedEntries =
            entries |> List.map (\entry -> ( entry.x - minX, entry.y - minY ))

        emptyBoard =
            List.repeat (maxY - minY + 1) (List.repeat (maxX - minX + 1) False)

        -- _ = Debug.log "" { minX = minX, minY = minY, maxX = maxX, maxY = maxY, me = mappedEntries}
    in
    mappedEntries
        |> List.foldl (\( x, y ) board -> board |> H.replaceAt y (H.replaceAt x (always True))) emptyBoard
        |> List.indexedMap
            (\i ->
                List.map
                    (\e ->
...
moveXTimes : Int -> List Entry -> List Entry
moveXTimes i input =
    case i of
        0 ->
            input
        ...
        _ ->
            let
                _ =
                    print input
            in
            moveXTimes (i - 1) (input |> List.map move)


moveUntilReasonable : ( Int, List Entry ) -> ( Int, List Entry )
moveUntilReasonable ( i, entries ) =
    let
        minX =
            entries |> List.map .x |> List.foldl min 1000000000
    ...
    case maxX - minX < 300 && maxY - minY < 300 of
        True ->
            entries |> always ( i, entries )

        False ->
            ( i + 1, entries |> List.map move ) |> moveUntilReasonable


res1 =
    input
        |> String.lines
        |> List.map (Parser.run parser >> H.uR)
        |> (\x -> moveUntilReasonable ( 0, x ))
