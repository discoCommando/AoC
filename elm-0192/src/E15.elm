...
type alias Cell =
    { state : Intcode.State, length : Int, position : Position, tile : Tile }
...
type alias State =
    { visited : Dict Position ( Int, Tile ), queue : Queue }


initialIState : Intcode.State
initialIState =
    { array = parsed, status = Intcode.Going 0, input = [], output = [], relativeBase = 0 }


initialState : State
initialState =
    { visited = Dict.empty, queue = Queue.singleton { position = ( 0, 0 ), length = 0, tile = Empty, state = initialIState } }
...
bfs state =
    let
        ...
        newVisited =
            state.visited |> Dict.insert cell.position ( cell.length, cell.tile )

        newState =
            { state | visited = newVisited, queue = newQ }
    in
    case cell.tile of
        Wall ->
            bfs newState
                        ...
                        |> List.filterMap
                            (\direction ->
                                let
                                    ...
                                    newPosition =
                                        move cell.position direction
                                in
                                case Dict.get newPosition state.visited of
                                    Just _ ->
                                        Nothing
                                    ...
                                    _ ->
                                        let
                                            ...
                                            newIState =
                                                { istate | input = [ directionToInt direction ], output = [] } |> Intcode.walk

                                            tile =
                                                newIState |> .output |> Helpers.at 0 |> intToTile
                                        in
                                        Just { tile = tile, state = newIState, position = newPosition, length = cell.length + 1 }
                            )

                newQueue =
                    List.foldl Queue.push newState.queue newCells
            in
            bfs { newState | queue = newQueue }


type alias Cell2 =
    { position : Position, state : Intcode.State, tile : Tile }
...
type alias State2 =
    { visited : Set Position, queue : Queue2 }


popUntil : Int -> State2 -> State2
popUntil counter state2 =
    if counter == 0 then
        state2
    else
        let
            ( cell, newQueue ) =
                state2.queue |> Queue.pop

            newState =
                { state2 | visited = state2.visited |> Set.insert cell.position, queue = newQueue }
        in
        case cell.tile of
            Wall ->
                popUntil (counter - 1) newState
                                ...
                                (\direction ->
                                    let
                                        ...
                                        newPosition =
                                            move cell.position direction
                                    in
                                    case Set.member newPosition state2.visited of
                                        True ->
                                            Nothing
                                        ...
                                        _ ->
                                            let
                                                ...
                                                tile =
                                                    newIState |> .output |> Helpers.at 0 |> intToTile
                                            in
                                            Just { tile = tile, state = newIState, position = newPosition }
                                )

                    newQueue2 =
                        List.foldl Queue.push newState.queue newCells
                in
                popUntil (counter - 1) { newState | queue = newQueue2 }


bfs2 : Int -> State2 -> Int
bfs2 counter state2 =
    case state2.queue |> Queue.size of
        0 ->
            counter - 1

        s ->
            bfs2 (counter + 1) (popUntil s state2)
...
initialState2 =
    { visited = Set.empty, queue = Queue.singleton { position = result1.position, tile = result1.tile, state = result1.state } }


result2 =
    bfs2 0 initialState2
